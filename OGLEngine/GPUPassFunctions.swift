...
struct GPUPassFunctions {
    ...
    static func mat2Pass(mat2: GLKMatrix2, location: GLint) {
        var mat2 = mat2
        withUnsafePointer(&mat2, {
            glUniformMatrix2fv(location, 1, 0, UnsafePointer($0))
        })
    }
    ...
    static func intPass(int: Int, location: GLint) {
    ...
    static func cubeTextureSidePass(texture: CubeTexture, side: CubeTextureSide, location: GLint) {
        let index = side.cubeContextIndex()
        glActiveTexture(GLenum(GL_TEXTURE0 + index));
        glBindTexture(side.glEnum(), texture.glName);
        glUniform1i(location, 0 + index);
    }
